import { tauriApi } from './js/tauriIpc.js';
import { pathToFileURL as resolveAssetURL } from './js/path.js';

// Phase 2 Tauri adapter shim: all 13 `window.api.*` call sites below work
// unchanged — in Tauri there is no preload, so provide the invoke-backed API.
// `window.api ??=` keeps Electron/window.api (and test mocks) intact.
...
function pathToFileURL(p) {
  // Phase 2: asset:// under Tauri (convertFileSrc), file:/// fallback otherwise.
  // Original encodeURI implementation lives in ./js/path.js (pathToFileURLFallback).
  ...
  if (typeof window !== 'undefined' && window.__TAURI__) {
    import('@tauri-apps/api/webview').then(({ getCurrentWebview }) => {
      getCurrentWebview().onDragDropEvent((event) => {
        ...
        if (type === 'drop' && event.payload.paths && event.payload.paths[0]) {
          dropOverlay.classList.remove('visible');
          openAndRenderFolder(event.payload.paths[0]).catch(() => {});
